...
def run_professional_backtest(ticker, period='1y', initial_capital=10000, position_pct=1.0, 
    ...
    for strategy in strategies:
        ...
        start_idx = max(0, min(14, len(prices) - 1))  # Dynamic start index bounded by dataset length
        
        for i in range(start_idx, len(prices)):
            current_price = prices.iloc[i]
            current_date = dates[i]
            current_rsi = rsi.iloc[i] if i < len(rsi) and not pd.isna(rsi.iloc[i]) else 50
            current_macd = macd_line.iloc[i] if i < len(macd_line) and not pd.isna(macd_line.iloc[i]) else 0
            current_signal = signal_line.iloc[i] if i < len(signal_line) and not pd.isna(signal_line.iloc[i]) else 0
...
def get_tickers_from_groq(description):
    ...
    prompt = (
        f"Search the web for recent stock news, earnings, analyst upgrades, catalysts, and sector momentum for: '{description}'. "
        "Return exactly 5 stock tickers separated by commas. No explanation, no extra text. "
        "Be open to all stocks including lesser-known ones - focus on current performance, momentum, and catalysts, not just famous well-known companies."
    )

    response = create_chat_completion(prompt, model="groq/compound-mini")
    raw = response.choices[0].message.content.strip()
    # Use case-insensitive regex to catch lowercase tickers like 'aapl'
    tickers = re.findall(r'\b[A-Za-z]{1,5}\b', raw)
    # Convert to uppercase for consistency
    tickers = [t.upper() for t in tickers]
    
    # Filter out common non-ticker words and validate tickers
    invalid_words = {'AND', 'THE', 'FOR', 'BUT', 'NOT', 'ARE', 'WAS', 'HAS', 'CAN', 'NOW', 'GET', 'USE', 'ALL', 'NEW', 'ONE', 'TWO', 'OUR', 'YOU', 'WHO', 'HOW', 'WHY', 'YES', 'TOP', 'BEST', 'NOW', 'SEE', 'JUST', 'LIKE', 'MORE', 'SOME', 'ALSO', 'ONLY', 'OVER', 'SUCH', 'INTO', 'YEAR', 'MOST', 'VERY', 'EVEN', 'BACK', 'AFTER', 'THEN', 'SOME', 'MANY', 'MUCH', 'WHEN', 'WELL', 'WAY', 'DAY', 'TODAY', 'THIS', 'THAT', 'WITH', 'FROM', 'HAVE', 'HAD', 'THEY', 'BEEN', 'WERE', 'WHICH', 'THERE', 'WOULD', 'COULD', 'SHOULD', 'MAY', 'WILL', 'JUST', 'BEING', 'OTHER', 'THINK', 'ABOUT', 'THESE', 'EACH', 'THEIR', 'WHAT', 'SO', 'DO', 'IF', 'OR', 'BY', 'ON', 'AT', 'IN', 'IS', 'IT', 'TO', 'OF', 'AS', 'AN', 'BE', 'MY', 'WE', 'US', 'HIS', 'HER', 'ITS', 'HE', 'SHE', 'ME', 'YOUR', 'THEM', 'ME', 'ME', 'ME'}
    valid_tickers = []
    
    for ticker in tickers[:10]:  # Check up to 10 to find 5 valid ones
        if ticker not in invalid_words:
